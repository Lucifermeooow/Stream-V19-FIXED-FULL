...
4. Open **Actions** → **Build Stream V19 APK**.
5. Download the `Stream-V19-APK` artifact after a successful build.
...
The previous build failed because `rtmp_streaming`/RootEncoder required Android API 37 while the project compiled against API 36. Stream V19 compiles against API 37 and uses AGP 9.0.1 + Gradle 9.3.1-compatible tooling. The Android toolchain is pinned for the Flutter 3.47 build environment, and the plugin itself now uses RootEncoder 2.8.0 in its 2.x line.
